...
const TableRowMemo = React.memo(({ row, isPinned, hasDetails, rowSelection }) => {
...
const TanTableBody = ({ table, hasDetails, rowSelection }) => {
...
TanTableBody.defaultProps = {
  hasDetails: false,
  rowSelection: {},
};

export const MemoTanTableBody = React.memo(
  TanTableBody,
  (prev, next) =>
...
const DataTableTanstack = ({
  ...
  // Calculate pinned source IDs: selection + details (regardless of presence in current data)
  ...
  // Build a map of current rows for quick access
  ...
  // Duplicate pinned rows (distinct id) using current rows or cache
  ...
  // Final dataset includes base rows + pinned duplicates so originals stay visible
  const tableData = useMemo(() => [...baseRows, ...pinnedRows], [baseRows, pinnedRows]);

  // Pin only the duplicated rows
  const validPinnedIds = useMemo(() => pinnedRows.map(row => String(row.id)), [pinnedRows]);

  // Transform Blueprint columns to TanStack columns format
  ...
  const columns = useMemo(() => {
    ...
    return [
      {
        id: 'select',
        // No header checkbox - individual selection only
        header: EmptyHeader,
  ...
  const table = useReactTable({
    ...
    state: {
      rowSelection,
      rowPinning: { top: validPinnedIds, bottom: [] }, // Use computed valid IDs directly
    },
    enableRowSelection: true,
    getRowCanSelect: () => true,
    enableColumnResizing: true,
    enableRowPinning: true,
    keepPinnedRows: true, // Keep this enabled for cross-page pinning
  ...
  // Reset selection when data changes to invalid rows
  useEffect(() => {
      ...
      <TableContainer component={Paper} sx={{ flex: 1, overflow: 'auto' }}>
        <Table size="small" stickyHeader sx={{ tableLayout: 'fixed', ...columnSizeVars }}>
                ...
                {headerGroup.headers.map((header, headerIndex) => {
                  ...
                  const renderHeaderContent = () => {
                    ...
                    if (header.column.id === 'select' || header.column.id === 'minifiche') {
                      // Empty header for checkbox and minifiche columns
          ...
          {table.getState().columnSizingInfo.isResizingColumn ? (
            <MemoTanTableBody table={table} hasDetails={hasDetails} />
          ) : (
            <TanTableBody table={table} hasDetails={hasDetails} />
          )}
        </Table>
      </TableContainer>
      <TablePagination
...
DataTableTanstack.defaultProps = {
  columns: [],
  rows: [],
  loading: false,
  rowSelection: {},
