...
const ReportingModule = ({
  ...
  const submitReport = async submitData => {
    try {
      ...
      formDataToSend.append('feature', featureId || '');
      // formDataToSend.append('layer', layer?.id || '');
  ...
  const renderFieldInput = field => {
    ...
    const { format_type, value, required, sourceFieldId } = field;
    ...
    return (
      <TextField
        ...
        type="text"
        variant="outlined"
        helperText={`Champ source : ${value} (${format_type})`}
        sx={{ mb: 2 }}
        multiline
        rows={2}
        required={required}
